...
class LiquibaseChangesetXMLParser implements LiquibaseChangesetParser, ILiquibaseParseChangelogFile {
    ...
    public void doRenameColumn() {
        ...
        Map<String, Column> columnsAsMap = table.getTable_columns().stream().collect(Collectors.toMap(Column::getColumn_name, c -> c));
        columnsAsMap.get(oldColumnName).setColumn_name(newColumnName);
    }
    ...
    public void doAddForeignKeyConstraint() {
        ...
        Map<String, Column> baseColumnsAsMap = baseTable.getTable_columns().stream().collect(Collectors.toMap(Column::getColumn_name, c -> c));
        Column baseColumn = baseColumnsAsMap.get(getCurrentElement().getAttributeValue("baseColumnNames"));

        Table referencedTable = getTableBasedOnCurrentElementAttributeValue("referencedTableName");
        Map<String, Column> referencedColumnsAsMap = referencedTable.getTable_columns().stream().collect(Collectors.toMap(Column::getColumn_name, c -> c));
        Column referencedColumn = referencedColumnsAsMap.get(getCurrentElement().getAttributeValue("referencedColumnNames"));

        baseColumn.setForeign_key_column(referencedColumn.getColumn_name());
        baseColumn.setForeign_key_table(referencedTable.getTable_name());
